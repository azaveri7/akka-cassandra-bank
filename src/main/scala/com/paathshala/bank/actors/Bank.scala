        ...
        event match {
          case BankAccountCreated(id) =>
            val account = context.child(id) // exists after command handler
              .getOrElse(context.spawn(PersistentBankAccount(id), id))// does NOT exist in the recovery mode
...
object BankPlayground {
  ...
  def main(args: Array[String]): Unit = {
    val rootBehavior: Behavior[NotUsed] = Behaviors.setup{ context =>
      ...
      bank ! CreateBankAccount("anand", "INR", 10, responseHandler)
      //bank ! GetBankAccount("<message-id>", responseHandler)
